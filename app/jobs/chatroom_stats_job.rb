class ChatroomStatsJob < ActiveJob::Base
  ...
  BLACKLIST_CHATROOM = %w(53ba02e7144452cd76000004 547eb6811444529ad8000091).freeze
  ...
  def perform(start_date, end_date)
    ...
    Net::SSH.start(IM_SERVER, IM_USER, password: IM_PWD, port: IM_PORT) do |ssh|
      ...
      until @end_date.to_date < current_date
        ...
        puts " -> #{current_date}"
        puts "    Fetching chatroom messages total: [#{start_timestamp}]"
        ...
        date_messages.each do |topic_id, item|
          # 艾普罗菲尔，穷游办公室跳过
          next if BLACKLIST_CHATROOM.include?topic_id
          # 不是城市聊天室的跳过
    ...
    @chatrooms.each do |_topic_id, chatroom|
      message_total = 0
  ...
  def city_chatrooms!
    Rails.cache.fetch(CITY_CHATROOMS_KEY) do
      ...
      Qyer::Chatroom.all.each do |c|
        # 艾普罗菲尔，穷游办公室
        next if BLACKLIST_CHATROOM.include?c.im_topic_id
